                    ...
                    <div class="center-menu-bottom">
                        <form class="form-inline row form-search" action="search" method="get">
                            <%
                                ...
                                String strCate = request.getAttribute("category_id").toString();
                                String nameSear = (String)request.getAttribute("name");
                                if(nameSear.length()==0) nameSear ="";
                                int cateID = Integer.parseInt(strCate);
                            %>
                            <select class="custom-select col-md-4" name="category_id">
                                <option value="0">Select Category</option>
                                <%
                                    for(Category list : listCate) {
                                        if(list.getId()==cateID) {
                                            %>
                                     <option selected value="<%=list.getId()%>"><%=list.getName()%></option>
                                <%
                                        } else {
                                            %>
                            ...
                            <input class="form-control col-md-5" name="name" value="<%=nameSear%>" type="search" placeholder="Search Book" aria-label="Search">
...
<%--        %>--%>
        ...
        <c:if test="${list!=null}">
            <c:forEach var="item" items="${list}">
                <div class="product">
                    <div class="image">
                        <img class="lazy" src="images/${item.picture}" style="width: 100%; display: inline;">
                    </div>
                    <a class="title" href="#">
                            ${item.name}
                    </a>
                    <div class="price-order">
                        ...
                        <div class="order">
                            <a class="order-add" href="addToCart?bookId=${item.id}">
                                <img src="http://cdn.onlinewebfonts.com/svg/img_553457.png" alt="" border="0" style="width: 70%; display: inline;">
                            </a>
                        </div>
                    </div>
